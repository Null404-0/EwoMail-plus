...
install_unicode_plugin() {
    ...
    # 创建成 root:root 0755（当前进程 umask）。这样 PHP-FPM(www-data) 后续要
    # 在 _default_/ 里 mkdir cache/ 就 Permission denied，SnappyMail bootstrap
    # 直接抛 RuntimeException → 登录页空 body 200。每一层显式 chown 一遍。
    for d in \
